    ...
    if (childname != '') {
        ...
        $.ajax({
            ...
            for (var i = 0; i < json.length; i++) {
                   var child_id     = json[i].id;
                   var child_name   = json[i].child_name;
                   var lists =lists+ '<li class="child_full_detail" data-childid="'+ child_id +'" data-title="'+ child_name +'">'+ child_name + '</li>';
                }
            var lists = lists+ '</ul>';
            $('.childs_names').html(lists);
...
// Get all child details 
$('body').on('click', '.child_full_detail', function () {
    var child_detail = $(this).data('childid');
        ...
        $.ajax({
            ...
            success: function (data) {
 				if (data == '1') {
 					...
 					$('#add_goal').find(':input').val('');
 					setTimeout(function(){ $('#responsive').modal('hide'); }, 2000); 					
 				} else {
	                $('.alert-danger').show();
	                $('#success-error').html('Try Again')
 				}
